    ...
    <section class="theme-section">
      ...
      <ul>
        <li class="theme-card" v-for="item in officialTheme" :key="item.name">
          <theme-card type="official" :config="item" @action="onAction"></theme-card>
        </li>
      </ul>
    </section>
    <section class="theme-section second-section">
      ...
      <ul>
        ...
        <li class="theme-card" v-for="item in displayUserTheme" :key="item.name">
          <theme-card type="user" :config="item" @action="onAction"></theme-card>
        </li>
      </ul>
    </section>
    <al-dialog :visible.sync="copyDialogVisible">
      <al-form :model="copyForm" ref="copyForm" :rules="copyFormRule">
        <al-form-item label="Theme name" prop="name">
          <al-input v-model="copyForm.name"></al-input>
        </al-form-item>
      </al-form>
      <div slot="footer" class="dialog-footer">
        <al-button @click="closeCopyForm">{{ getActionDisplayName('cancel') }}</al-button>
        <al-button type="primary" @click="copyToUser">{{
          getActionDisplayName('confirm')
        }}</al-button>
      </div>
    </al-dialog>
  </div>
...
export default {
  ...
  computed: {
    officialTheme() {
      ...
      return this.padEmpeyTheme(themeList.concat([]));
    },
    userThemeCount() {
  ...
  methods: {
    ...
    onAction(name, item) {
      switch (name) {
        ...
        case 'delete':
          this.$confirm(
            this.getActionDisplayName('confirm-delete-theme'),
            this.getActionDisplayName('notice'),
            {
