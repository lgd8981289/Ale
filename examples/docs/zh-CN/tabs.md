...
<al-tabs type="border-card">
  <al-tab-pane>
    <span slot="label"><i class="al-icon-date"></i> 我的行程</span>
    我的行程
  </al-tab-pane>
  <al-tab-pane label="消息中心">消息中心</al-tab-pane>
  <al-tab-pane label="角色管理">角色管理</al-tab-pane>
  <al-tab-pane label="定时任务补偿">定时任务补偿</al-tab-pane>
</al-tabs>
```
...
```html
<al-tabs v-model="editableTabsValue" type="card" editable @edit="handleTabsEdit">
  <al-tab-pane
    :key="item.name"
...
<div style="margin-bottom: 20px;">
  <al-button size="small" @click="addTab(editableTabsValue)">
    add tab
  </al-button>
</div>
<al-tabs v-model="editableTabsValue" type="card" closable @tab-remove="removeTab">
  <al-tab-pane
    v-for="(item, index) in editableTabs"
    :key="item.name"
    :label="item.title"
