  ...
  setup(props, { slots }) {
    ...
    ['span', 'offset', 'pull', 'push'].forEach(prop => {
      if (props[prop] || props[prop] === 0) {
        classList.push(prop !== 'span' ? `al-col-${prop}-${props[prop]}` : `al-col-${props[prop]}`);
      }
    });
    ['xs', 'sm', 'md', 'lg', 'xl'].forEach(size => {
      if (typeof props[size] === 'number') {
        classList.push(`al-col-${size}-${props[size]}`);
      } else if (typeof props[size] === 'object') {
        let iProps = props[size];
        Object.keys(iProps).forEach(prop => {
          classList.push(
            prop !== 'span'
              ? `al-col-${size}-${prop}-${iProps[prop]}`
              : `al-col-${size}-${iProps[prop]}`
          );
        });
      }
    });
