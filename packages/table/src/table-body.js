...
export default {
  ...
  render(h) {
    ...
    return (
      <table class="al-table__body" cellspacing="0" cellpadding="0" border="0">
        <colgroup>
  ...
  watch: {
    ...
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.al-table__row');
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      const classes = ['al-table__row'];
      if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
        classes.push('current-row');
      }

      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('al-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'al-tooltip') && cellChild.childNodes.length)) {
        return;
      }
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push('al-table__row--leval-' + treeRowData.level);
        ...
        return [
          [
            ...
            <tr key={'expanded-row__' + tr.key}>
              <td colspan={this.columnsCount} class="al-table__expanded-cell">
