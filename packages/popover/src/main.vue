<template>
  <span>
    <transition :name="transition" @after-enter="handleAfterEnter" @after-leave="handleAfterLeave">
      <div
        class="al-popover al-popper"
        :class="[popperClass, content && 'al-popover--plain']"
        ref="popper"
      ...
      >
        <div class="al-popover__title" v-if="title" v-text="title"></div>
        <slot>{{ content }}</slot>
      </div>
    </transition>
...
export default {
  ...
  mounted() {
    ...
    // 可访问性
    if (reference) {
      addClass(reference, 'al-popover__reference');
